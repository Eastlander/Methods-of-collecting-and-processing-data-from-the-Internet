...
class InstaPhotoPipeline(ImagesPipeline):
    ...
    # переопределим путь к изображениям (images/таргет-пользователь/статус отношений/file_name.jpg)
    ...
    def item_completed(self, results, item, info):
        for result in [x for ok, x in results if ok]:
            ...
            # если пути к папке не существует - создаём
...
class InstaPipeline:
    ...
    def process_item(self, item, spider):
        # т.к. всю информацию собрали, предварительная обработка полей не нужна
        # будем сразу скидывать в БД
